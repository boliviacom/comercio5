...
export class ColumnConfigDashboard {
        ...
        return `
            <div class="table-card" data-table-name="${tableName}">
                ...
                <div class="card-footer button-group">
                    <button class="btn-manage-columns" data-table-name="${tableName}">
                        <i class="fas fa-columns"></i> Configurar Columnas
                    </button>
                    <button class="btn-manage-data" data-table-name="${tableName}">
                        <i class="fas fa-tasks"></i> Gestionar Datos
                    </button>
                </div>
            </div>
        `;
    ...
    loadConfigurationPanel() {
        ...
        document.querySelectorAll('.btn-manage-columns').forEach(button => {
        ...
        document.querySelectorAll('.btn-manage-data').forEach(button => {
            button.addEventListener('click', (e) => {
                const tableName = e.currentTarget.getAttribute('data-table-name');
                this.handleManageDataClick(tableName);
            });
        });
    }

    handleManageDataClick(tableName) {
        if (!REPORT_CONFIG[tableName]) {
            alert(`No hay configuración de informe definida para la tabla: ${tableName}.`);
            return;
        }
